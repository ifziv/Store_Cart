...
//显示照片
@property (nonatomic,retain) UIImageView *spImageView;
//商品名
...
-(instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    ...
    if (self) {
        self.backgroundColor = ZVColorFromRGB(245, 246, 248);
...
- (void)initMainView {
    ...
    bgView.frame = CGRectMake(10, 10, ZVSCREEN_WIDTH - 20, ZVTAG_CartEmptyView - 10);
    bgView.backgroundColor = [UIColor whiteColor];
    bgView.layer.borderColor = ZVColorFromHex(0xEEEEEE).CGColor;
    ...
    imageBgView.backgroundColor = ZVColorFromHex(0xF3F3F3);
    ...
    priceLabel.textColor = ZVBASECOLOR_RED;
    ...
    sizeLabel.textColor = ZVColorFromRGB(132, 132, 132);
    ...
    dateLabel.textColor = ZVColorFromRGB(132, 132, 132);
...
- (void)reloadDataWithModel:(CartGoodsModel*)model {
    ...
    self.spImageView.image = model.image;
    self.nameLabel.text = model.goodsName;
    self.priceLabel.text = [NSString stringWithFormat:@"¥%@", model.realPrice];
    self.dateLabel.text = [NSString stringWithFormat:@"¥%@", model.orginalPrice];
...
- (void)setCartNumber:(NSInteger)cartNumber
{
    ...
    self.numberLabel.text = [NSString stringWithFormat:@"%ld",(long)cartNumber];
}

- (void)setCartSelected:(BOOL)cartSelected
{
    _cartSelected = cartSelected;
    self.selectBtn.selected = cartSelected;
}
